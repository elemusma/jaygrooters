...
@referers = ('www.jaygrooters.com','jaygrooters.com','72.37.160.5');

# Path to the file that specifies the list of email addresses.
# Typically, you need to create a text file named 'allowedRecipients'
# and fill in the list of email addresses with each address
# in a separate line.
# The script can send emails to these addresses only
# Remember you should provide full read permissions for this file.
# If you using Unix/Linux, you may type: chmod 777 allowedRecipients
...
sub parse_form {
    ...
    foreach $pair (@pairs) {
        ...
        if (defined($Config{$name})) {
            $Config{$name} = $value;
		###>> this gets rid of stupid hack crap
               	$Config{$name}=~s/(\s+bcc:)|(^bcc:)|(\cMbcc:)|(\s+to:)|(^to:)|(\cMto:)|(\sfrom:)|(^from:)|(\cMfrom:)//gi;
        }
        else {
            if ($Form{$name} && $value) {
                $Form{$name} = "$Form{$name}, $value";
		###>> this gets rid of stupid hack crap
                $Form{$name}=~s/(\s+bcc:)|(^bcc:)|(\cMbcc:)|(\s+to:)|(^to:)|(\cMto:)|(\sfrom:)|(^from:)|(\cMfrom:)//gi;
            }
            elsif ($value) {
...
sub return_html {
    ...
    # Otherwise, begin printing the response page.                           #
        ...
        <center><font size=-1>FormMail V1.6 &copy; 1995 -1997  Matt Wright<br>
  ...
  <center>
   ...
   <table border=0 width=600 bgcolor=#CFCFCF>
    <tr><td>The form attempting to use
     ...
     <center><font size=-1>
      FormMail V1.6 &copy; 1995 - 1997  Matt Wright<br>
      A Free Product of Matt's Script Archive, Inc.
  ...
  <center>
   ...
   <table border=0 width=600 bgcolor=#CFCFCF>
    <tr><th><tt><font size=+1>Copyright 1995 - 1997 Matt Wright<br>
        ...
        A Free Product of Matt's Script Archive,
        Inc.
<br>
     <a href="http://www.mailvalley.com/formmail">Anti-spam and Security fix</a>
     added by Parameshwar Babu (babuWeb\@mailvalley.com).
...
(END ERROR HTML)
    ...
    elsif ($error eq 'cantget_recipient') {
            print <<"(END ERROR HTML)";
Content-type: text/html

<html>
 <head>
  <title>Error: Cannot Access Recipient File</title>
 </head>
 <body bgcolor=#FFFFFF text=#000000>
  <center>
   <table border=0 width=600 bgcolor=#9C9C9C>
    <tr><th><font size=+2>Error: Cannot Access Recipient File</font></th></tr>
   </table>
   <table border=0 width=600 bgcolor=#CFCFCF>
    <tr><td> The allowedRecipients file is not accessible. Please check the file permission.
    <hr size=1>
