...
var commentsBox, pm_sanitize, WPSetThumbnailHTML, WPSetThumbnailID, WPRemoveThumbnail, wptitlehint, makeSlugeditClickable, editPermalink;
	...
	if ( $('#submitdiv').length ) {
		...
		updateText = function() {
			...
			$('#post-status-display').text(
				pm_sanitize.stripTagsAndEncodeText( $('option:selected', postStatus).text() ) // Remove any potential tags from post status text.
			);
